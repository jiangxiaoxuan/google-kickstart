## Question 3
```cpp
//I didn’t manage to find a correct answer for this solution, this is an easy to understand solution and I have tested on the day of trial round.
#include <stdlib.h>
#include <iostream>
#include <vector>
...
int main() {
    ...
    for (int tt=1;tt<=no;tt++){
        ...
        long long ans=0;
        for (int i=0;i<n;i++){
            ans=(ans+((((n-i)*a[i])%MOD)*sum[i])%MOD)%MOD;
        }
